...
## DHIS2 Name Standardization

The app uses DHIS2 as the "source of truth" for admin area names:

- **DHIS2 Reference Builder** - Separate tool located at `../dhis2_reference_builder/`
- **Reference Files** - CSV files in `assets/dhis2_*.csv` (generated by builder tool)
- **Cleaning Integration** - `R/cleaning_functions.R` automatically loads DHIS2 references
- **Fallback** - Uses hardcoded mappings if DHIS2 files unavailable

To update DHIS2 references, use the separate builder tool, not the Shiny app.